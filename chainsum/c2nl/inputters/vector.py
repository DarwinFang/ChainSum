...
def vectorize(ex, model):
    ...
    vectorized_ex['code_edge_rep'] = None
    ...
    if model.args.use_graph:
        vectorized_ex['code_edge_rep'] = torch.LongTensor(code.edges)
    ...
    vectorized_ex['use_graph'] = model.args.use_graph
...
def batchify(batch):
    ...
    code_edges = [ex['code_edge_rep'] for ex in batch]
    ...
    for i in range(batch_size):
        ...
        if use_graph:
            e_num = code_edges[i].size(1)
            edge_num_rep[i] = e_num
            code_edge_rep[i, :, :e_num].copy_(code_edges[i])
        if use_code_type:
