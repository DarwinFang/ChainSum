...
from c2nl.inputters.constants import BOS_WORD, EOS_WORD, PAD_WORD, \
    UNK_WORD, TOKEN_TYPE_MAP, AST_TYPE_MAP, DATA_LANG_MAP, LANG_ID_MAP, MAX_EDGENUM_PER_NODE
...
def filter_columns(array, threshold):
    np_array = np.array(array)
    columns_to_keep = [col for col in range(np_array.shape[1]) if not ((np_array[0, col] >= threshold) or (np_array[1, col] >= threshold))]
    return np_array[:, columns_to_keep].tolist()

def my_process_examples(lang_id,
                     data,
                     max_src_len,
                     max_tgt_len,
                     uncase=False,
    ...
    max_edge_num = int(max_src_len * MAX_EDGENUM_PER_NODE)
    tokens = data['tokens'][:max_src_len]
    summ = data['text']
    if len(tokens) * len(summ) == 0:
        return None
    if not test_split:
        summ = summ[:max_tgt_len]
    if uncase:
        summ = [t.lower() for t in summ]

    code = Code()
    code.text = data['code']
    code.language = lang_id
    code.tokens = tokens
    # code.edges = [p[:max_edge_num] for p in filter_columns(data['edges'], len(tokens))]
    edge_src, edge_tgt = [p[:max_edge_num] for p in filter_columns(data['edges'], len(tokens))]
    code.edges = [edge_src + edge_tgt, edge_tgt + edge_src]
    code.type = data['tags'][:max_src_len]
    summary = Summary()
    summary.text = ' '.join(summ)
    summary.tokens = summ
    summary.prepend_token(BOS_WORD)
    summary.append_token(EOS_WORD)
...
def my_load_data(args, filename, max_examples=-1, dataset_name='java',
              test_split=False):
    dataset = json.load(open(filename, 'r'))
    examples = []
    if dataset_name in ['java', 'python']:
        for data in tqdm(dataset.values(), total=len(dataset)):
            _ex = my_process_examples(LANG_ID_MAP[DATA_LANG_MAP[dataset_name]],
                                      data,
                                      args.max_src_len,
                                      args.max_tgt_len,
                                      uncase=args.uncase,
