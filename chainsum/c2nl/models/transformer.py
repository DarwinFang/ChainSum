...
from c2nl.encoders.g_encoder import G_Encoder
...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        if self.use_type:
            ...
            #                                     self.enc_input_size)
            self.type_embeddings = nn.Embedding(constants.TOKEN_TYPE_NUM,
                                                self.enc_input_size)
    ...
    def forward(self,
        ...
        if mode == 'encoder':
            ...
            if self.use_type:
                ...
                word_rep = word_rep + self.lambda_t * type_rep
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        if self.use_graph:
            self.encoder = G_Encoder(args, self.embedder.enc_input_size, self.encoder)
        self.decoder = Decoder(args, self.embedder.dec_input_size)
        # self.layer_wise_attn = args.layer_wise_attn
        self.layer_wise_attn = args.layer_wise_attn if not self.use_graph else False
    ...
    def _run_forward_ml(self,
        ...
        code_rep = self.embedder(code_word_rep,
                                 ...
                                 code_type_rep,
                                 mode='encoder')
        layer_wise_outputs = None
        if self.use_graph:
            memory_bank = self.encoder(code_rep, code_len, code_edge_rep, edge_num)
        else:
    ...
    def decode(self,
        ...
        layer_wise_outputs = None
        if self.use_graph:
            memory_bank = self.encoder(word_rep, code_len, code_edge_rep, edge_num)
        else:
