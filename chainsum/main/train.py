...
def add_train_args(parser):
    ...
    runtime.add_argument('--data_workers', type=int, default=4,
    ...
    files.add_argument('--train_file', nargs='+', type=str,
                       help='Preprocessed train source file')
    files.add_argument('--dev_file', nargs='+', type=str, required=True,
                       help='Preprocessed dev source file')
    files.add_argument('--test_file', nargs='+', type=str,
    ...
    general.add_argument('--with_tags', type=int, default=0,
                         help='Use tags as additional info')
    general.add_argument('--with_graphs', type=int, default=0,
                         help='Use graphs as additional info')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        if num_dataset > 1:
            if len(args.train_file) == 1:
                args.train_file = args.train_file * num_dataset

        for i in range(num_dataset):
            dataset_name = args.dataset_name[i]
            data_dir = os.path.join(args.data_dir, dataset_name)
            train_path = os.path.join(data_dir, args.train_file[i])
            if not os.path.isfile(train_path):
                raise IOError('No such file: %s' % train_path)

            args.train_files.append(train_path)
    ...
    num_dataset = len(args.dataset_name)
    if num_dataset > 1:
        args.dev_file = args.dev_file * num_dataset
        args.test_file = args.test_file * num_dataset

    for i in range(num_dataset):
        dataset_name = args.dataset_name[i]
        data_dir = os.path.join(args.data_dir, dataset_name)
        dev_path = os.path.join(data_dir, args.dev_file[i])
        test_path = os.path.join(data_dir, args.test_file[i])
        if not os.path.isfile(dev_path):
            raise IOError('No such file: %s' % dev_path)
        if not os.path.isfile(test_path):
            raise IOError('No such file: %s' % test_path)

        args.dev_files.append(dev_path)
        args.test_files.append(test_path)
...
def init_from_scratch(args, train_exs, dev_exs, test_exs):
    ...
    # Build a dictionary from the data questions + words (train/dev/test splits)
    logger.info('-' * 100)
    logger.info('Build word dictionary')
    src_dict = util.build_word_and_char_dict(args,
                                             # examples=train_exs + dev_exs,
                                             examples=train_exs + dev_exs + test_exs,
                                             fields=['code'],
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_file, dataset_name in zip(args.train_files, args.dataset_name):
            exs = util.my_load_data(args,
                                    train_file,
    ...
    dev_exs = []
    for dev_file, dataset_name in zip(args.dev_files, args.dataset_name):
        exs = util.my_load_data(args,
                                dev_file,
                                max_examples=args.max_examples,
                                dataset_name=dataset_name,
    ...
    test_exs = []
    for test_file, dataset_name in zip(args.test_files, args.dataset_name):
        exs = util.my_load_data(args,
                                test_file,
                                max_examples=args.max_examples,
                                dataset_name=dataset_name,
    ...
    test_dataset = data.CommentDataset(test_exs, model)
    ...
    if args.only_test:
        ...
        validate_official(args, test_loader, model, stats, mode='test')
    ...
    # TRAIN/VALID LOOP
        ...
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            result = validate_official(args, dev_loader, model, stats, mode='dev')
...
if __name__ == '__main__':
    ...
    set_defaults(args)
    args.use_code_type = bool(args.with_tags)
    args.use_graph = bool(args.with_graphs)
    # Set cuda
